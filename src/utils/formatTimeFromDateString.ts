...
export function getTimeAgoText(
    ...
    if (minutes < 1) {
        ...
        //`${hours} часа(ов) назад`;
        return `${hours} ${setTimeTranslation(
            `hours.${pluralizeLang(hours)}`
        )}`;
    } else if (days < 30) {
        //`${days} дня(ей) назад`;
        ...
        return `${months} ${setTimeTranslation(
            `months.${pluralizeLang(months)}`
        )}`;
    } else {
        //`${years} год(а) назад`;
        return `${years} ${setTimeTranslation(
            `years.${pluralizeLang(years)}`
        )}`;
    }
}
